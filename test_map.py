...
def test(cfg,
         ...
         batch_size=16,
         img_size=416,
         iou_thres=0.5,  #IOU的阈值
         conf_thres=0.001,
         nms_thres=0.5,  #非极大值抑制的阈值
         save_json=False,
         model=None):
    ...
    for batch_i, (imgs, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        #si是编号，pred是列表内容。  enumerate函数将一个可遍历的数据对象组合为一个索引序列。
    ...
    test_result = pf % ('all', seen, nt.sum(), mp, mr, map, mf1)
    print(test_result)

    # Print results per class
    if verbose and nc > 1 and len(stats):
        with open('ap_preClass.txt','w') as f:
            f.write('all: ' + str(map) + '\n')
            for i, c in enumerate(ap_class):
                print(pf % (names[c], seen, nt[c], p[i], r[i], ap[i], f1[i]))
                f.write(names[c] + ': ' + str(ap[i]) + '\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img-size', type=int, default=608, help='inference size (pixels), height and width')
