...
hyp = {'giou': 1.582,  # giou loss gain
       ...
       'weight_decay': 0.0009,  # optimizer weight decay
       # 'weight_decay': 0.001,
...
def train(cfg,
    ...
    # Initialize model = yolov3-spp.cfg
    ...
    start_epoch = 0
    best_fitness = 0.
    #从上次断掉的迭代期继续迭代，使用last.pt，并且在results.txt中延续写入训练结果，更新best.pt和start_epoch
    ...
    #加载预训练权重，并且判断权重文件与模型文件是否对应，不对应的删掉，只保留对应部分的预训练权重
    # elif (weights + 'yolov4-pacsp.pt').endswith('.pt'):  # pytorch format
    #     chkpt = torch.load(weights + 'yolov4-pacsp.pt', map_location=device)  # 预训练模型的全部参数
    #     print("yolov4-pacsp.pt")
    #     # load model
    #     try:
    #         model_dict = model.state_dict()  # 整个模型的结构
    ...
    #使用YOLOv3和YOLOv4的预训练权重
    else:  # Initialize model with backbone (optional)
        if '-tiny.cfg' in cfg:
            cutoff = load_darknet_weights(model, weights + 'yolov3-tiny.conv.15')
        else:
            #cutoff = 75，表示darknet53的最后一个卷积层在cfg文件中是第75层
            cutoff = load_darknet_weights(model, weights + 'darknet53.conv.74')

        print('Training from stracth!')
        pass
    ...
    # scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[round(opt.epochs * x) for x in [0.76, 0.84, 0.92]], gamma=0.1)
    ...
    dataset = LoadImagesAndLabels(train_path,
                                  ...
                                  rect=opt.rect,  # rectangular training, rect=False
                                  image_weights=opt.img_weights, #image_weights = False
                                  cache_images=opt.cache_images)  #cache_images = False
                                  #cache_images=True)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        #数据读取进度条，trainImages/batch_size向上取整（例如8187个训练图像，batch_size为16，则共有512个batch需要读取）
        pbar = tqdm(enumerate(dataloader), total=nb)  # progress bar
        for i, (imgs, targets, paths, _) in pbar:
            imgs = imgs.to(device)
            targets = targets.to(device)

            # Multi-Scale training，延续了yolov2中采用的每10个epoch，更换一次图像分辨率，图像大小必须为32的倍数，在这里不用
        ...
        # mAP是在一个训练epoch结束后，在验证集上计算得到的精度，会调用test.py，里面的参数分别为：
        "模型cfg：yolov3-spp； data：coco.data； batch_size：16； img_size：608； model：yolov3-spp； conf_thres：0.5"
...
if __name__ == '__main__':
    # kmean_anchors('/media/hdc/data3/lzk_ur_main/20201125_ODexps/yolov3-xmjEdition/coco/trainLabels.txt', 9, (608, 608), 0.6)
    # input('wait')
    ...
    parser.add_argument('--accumulate', type=int, default=8, help='number of batches to accumulate before optimizing')  #accumulate every 64 batches
