...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=True, image_weights=False, cache_images=False):
        ...
        #img_files就是trainImages内的图片数据
        n = len(self.img_files)
        #np.floor(x)：返回小于等于x的最大整数
        ...
        # Define labels
        "在将image_file中的图片转换为对应的标签时，需要先判断它是trainImage还是validImage"
        split = self.img_files[0].split("/")
        if split[8] == 'trainImages':
            self.label_files = [x.replace("trainImages","trainLabels").replace(os.path.splitext(x)[-1], '.txt')
                                for x in self.img_files]
        elif split[8] == 'validImages':
            self.label_files = [x.replace("validImages", "validLabels").replace(os.path.splitext(x)[-1], '.txt')
                                for x in self.img_files]
        elif split[8] == 'kmeans_img':
            self.label_files = [x.replace("kmeans_img", "kmeans_txt").replace(os.path.splitext(x)[-1], '.txt')
                                for x in self.img_files]
        else:
            self.label_files = [x.replace("testImages", "testLabels").replace(os.path.splitext(x)[-1], '.txt')
                                for x in self.img_files]
        ...
        if self.rect:
            ...
            s = np.array(s, dtype=np.float64) #s中是原始图像的尺寸，[width, height]
            ar = s[:, 1] / s[:, 0]  # aspect ratio， 高宽比
            i = ar.argsort()  #argsort将numpy数组的值从小到大排序，保存其下标值，返回值也为数组
        ...
        if augment or image_weights:  # cache labels for faster training
            ...
            for i, file in enumerate(pbar):
                try:
                    ...
                    # file = os.path.join('/media/hdc/data2/xmj/yolov3-master/train/annocations',file)
                    # file = '/media/hdc/data2/xmj/yolov3-master/train/annocations/'+file
                ...
                if l.shape[0]:
                    ...
                    if extract_bounding_boxes:
                        ...
                        "此时p只是****.jpg，读取图片和上述标签一样，需要添加前置地址，/media/hdc/data2/xmj/yolov3-master/train/JPEGImages/****.jpg"
                        # p = '/media/hdc/data2/xmj/yolov3-master/train/JPEGImages/' + str(p)
                        img = cv2.imread(p)
                        h, w, _ = img.shape
    ...
    def __getitem__(self, index):
        ...
        img = self.imgs[index]
        if img is None:
            # img_path = '/media/hdc/data2/xmj/yolov3-master/train/JPEGImages/' + img_path
            img = cv2.imread(img_path)  # BGR
            assert img is not None, 'Image Not Found ' + img_path
            r = self.img_size / max(img.shape)  # size ratio
            if self.augment and r < 1:  # if training (NOT testing), downsize to inference shape
        ...
        # Letterbox
        h, w, _ = img.shape
        "训练阶段self.rect=False，会变换为square正方形" \
        "test_map阶段self.rect=True，会变换为rect矩形" \
        "detect阶段不执行LoadImagesAndLabels，执行LoadImages，会变换为auto"
        if self.rect:
            shape = self.batch_shapes[self.batch[index]]
            img, ratiow, ratioh, padw, padh = letterbox(img, new_shape=shape, mode='rect')
            # img, ratiow, ratioh, padw, padh = letterbox(img, new_shape=self.img_size, mode='auto')
        else:
            shape = self.img_size
        ...
        # Load labels
        labels = []
        "label_path只是****.txt，需要添加前置地址"
        # label_path = '/media/hdc/data2/xmj/yolov3-master/train/annocations/' + label_path
