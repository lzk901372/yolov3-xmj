...
#计算网络模型的相关信息，包括含有需要更新参数的层的个数，以及参数量
...
#根据NMS和iou_thres筛选后得到的检测框计算每个类别的p，r，ap
def ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    for c in unique_classes:
        ...
        if n_p == 0 and n_gt == 0:
            ...
            # ax.set_xlim(0, 1)
            # fig.tight_layout()
            # fig.savefig('PR_curve%g.png' %c, dpi=300)
...
"path是进行聚类计算的所有样本； n是想要的聚类点； img_size是训练时样本的大小"
def kmeans_targets(path='./data/coco_64img.txt', n=9, img_size=320):  # from utils.utils import *; kmeans_targets()
    ...
    # Read shapes
    label_files = []
    nf = len(img_files)     #nf是图像的个数
    assert nf > 0, 'No images found in %s' % path
    for x in img_files:
        x = x.split('kmeans_img')[0] + x.split('/')[7].replace('img','txt') + '/' + x.split('/')[8].replace('jpg','txt')
    ...
    for i, file in enumerate(iter):
        try:
            #读取txt文件的内容
            with open(file, 'r') as f:
                #l中存放的是缺陷种类和中心坐标和宽高关于图像宽高的比例
                l = np.array([x.split() for x in f.read().splitlines()], dtype=np.float32)
                if l.shape[0]:
                    assert l.shape[1] == 5, '> 5 label columns: %s' % file  #判断该图像缺陷部位有没有1个类别+4个坐标
                    assert (l >= 0).all(), 'negative labels: %s' % file
                    assert (l[:, 1:] <= 1).all(), 'non-normalized or out of bounds coordinate labels: %s' % file    #判断4个坐标有没有归一化
                    l[:, [1, 3]] *= s[i][0]  # 计算出目标在原始图像中的中心点真实坐标，s[i][0]=width
                    l[:, [2, 4]] *= s[i][1]  # 计算出目标在原始图像中的宽和高真实值，s[i][1]=height
                    l[:, 1:] *= img_size / max(s[i])  #所有坐标都按照大的缩放比例计算
                    # l[:, [1, 3]] *= img_size[0] / s[i][0]  # 中心点的横坐标和检测框的宽按照宽的缩放比例计算
                    # l[:, [2, 4]] *= img_size[1] / s[i][1]  # 中心点的纵坐标和检测框的高按照高的缩放比例计算
                    labels[i] = l
        except:
...
def plot_results(start=0, stop=0):  # from utils.utils import *; plot_results()
    ...
    fig.tight_layout()
    ax[4].legend()
    fig.savefig('results%g.png' %stop, dpi=600)

def plot_loss(start=0, stop=0):  # from utils.utils import *; plot_results()
    # Plot training results files 'results*.txt'
    fig, ax = plt.subplots(1, 3, figsize=(14, 5))
    ax = ax.ravel()
    s = ['GIoU', 'Confidence', 'Classification']
    label = ['IoU Loss', 'Conf Loss', 'Cls Loss']
    for f in sorted(glob.glob('results*.txt') + glob.glob('../../Downloads/results*.txt')):
        results = np.loadtxt(f, usecols=[2, 4, 5, 9, 10, 13, 14, 15, 11, 12]).T
        n = results.shape[1]  # number of rows
        x = range(start, min(stop, n) if stop else n)
    return results[0,x], results[1,x], results[2,x]
...
def version_to_tuple(version):
    ...
    # return tuple(map(int, (version.replace('+cu92','').split('.'))))
